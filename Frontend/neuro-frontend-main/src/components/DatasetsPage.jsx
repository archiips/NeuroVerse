...
const DatasetsPage = () => {
  ...
  const fetchDatasets = async () => {
    try {
      ...
      // Map backend data to match your original structure
      const mappedDatasets = response.data.map(dataset => ({
        id: dataset.id,
        title: dataset.name || dataset.openneuro_id,
        description: dataset.description || `OpenNeuro Dataset ${dataset.openneuro_id}`,
    ...
    return (
      <div className="container mx-auto px-4 py-12 sm:px-6 lg:px-8 mt-6">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="animate-pulse flex-col overflow-hidden rounded-lg border border-gray-200/80 bg-white dark:border-dark-border/80 dark:bg-dark-border/30">
